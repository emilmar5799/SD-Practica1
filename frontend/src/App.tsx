...
const NodeCard = ({ node, selected, onClick }: any) => {
  ...
  const diskCount = node.disks?.length ?? 1;
  const pctDisk   = node.pctUse || 0;                  // server ya agrega el total
    ...
    <div onClick={() => onClick(node.nodeId)} style={{
      ...
      <div style={{ marginBottom: 6 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#94a3b8', marginBottom: 3 }}>
          <span>DISCO{diskCount > 1 ? <span style={{ color: '#38bdf8' }}> ×{diskCount}</span> : ''}</span>
...
const DetailPanel = ({ node, history, onCommand }: any) => {
  ...
  return (
    <div style={{
      ...
      {/* Detalle por disco (solo si hay 2 o más) */}
      {node.disks && node.disks.length > 1 && (
        <div style={{ background: '#0d1829', borderRadius: 12, padding: 12, border: '1px solid #1e293b' }}>
          <div style={{ fontSize: 10, color: '#64748b', marginBottom: 10 }}>
            DISCOS — {node.disks.length} unidades
          </div>
          {node.disks.map((d: any) => (
            <div key={d.diskName} style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#94a3b8', marginBottom: 3 }}>
                <span>
                  <span style={{ color: '#e2e8f0', fontWeight: 600 }}>{d.diskName}</span>
                  <span style={{ color: '#475569' }}> {d.diskType}</span>
                </span>
                <span style={{ color: pctColor(d.pctUse || 0) }}>
                  {d.usedGB}GB / {d.totalGB}GB · {d.pctUse || 0}%
                </span>
              </div>
              <GaugeBar value={d.pctUse || 0} color="#38bdf8" />
            </div>
          ))}
        </div>
      )}
...
// ── IDs válidos de los 9 departamentos de Bolivia ────────────────────────────
const DEPARTAMENTOS = ['lpz-01','cbba-01','scz-01','oru-01','pot-01','suc-01','tar-01','ben-01','pan-01'];
...
export default function App() {
  ...
  const { nodes: allNodes, loading } = useNodes(interval);
  const nodes   = allNodes.filter((n: any) => DEPARTAMENTOS.includes(n.nodeId));
